...
/** Keep LIFF session until the user logs out (cookie ≈ 400 days). */
export const LINE_AUTH_MAX_AGE = 60 * 60 * 24 * 400;
...
function secret() {
  return `${process.env.AUTH_PASSWORD ?? ""}:line-session`;
}
...
export async function createLineSessionToken(input: Omit<LineSession, "expiresAt">) {
  ...
  const payload = Buffer.from(
    JSON.stringify({ ...input, expiresAt: Date.now() + LINE_AUTH_MAX_AGE * 1000 }),
  ).toString("base64url");
  return `${payload}.${await sign(payload)}`;
}

export async function readLineSessionToken(token?: string): Promise<LineSession | null> {
  if (!token || !process.env.AUTH_PASSWORD) return null;
  const [payload, signature, extra] = token.split(".");
  if (!payload || !signature || extra || signature !== (await sign(payload))) return null;
  try {
    const value = JSON.parse(Buffer.from(payload, "base64url").toString("utf8")) as LineSession;
    return value.connectionId && value.loginId && value.lineUserId && value.expiresAt > Date.now()
      ? value
      : null;
  } catch {
    return null;
