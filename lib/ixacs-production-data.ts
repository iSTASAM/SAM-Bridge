...
import { todayBangkok, type ProductionAiDateQuery } from "@/lib/production-ai-query";

export type ProductionDataPayload = Record<string, unknown>;
type Discovery = Awaited<ReturnType<typeof discoverIxacsLines>>;
type DateQuery = ProductionAiDateQuery & { fresh?: unknown; customerIds?: unknown };

const DISCOVERY_TTL_MS = 5 * 60 * 1000;
type DataGlobals = typeof globalThis & {
  __ixacsDataDiscoveryCache?: Map<string, { expiresAt: number; value: Discovery }>;
  __ixacsProductionDataCache?: Map<string, { expiresAt: number; payload: ProductionDataPayload }>;
};
const shared = globalThis as DataGlobals;
const discoveryCache: Map<string, { expiresAt: number; value: Discovery }> =
  shared.__ixacsDataDiscoveryCache ??= new Map();
const dataCache: Map<string, { expiresAt: number; payload: ProductionDataPayload }> =
  shared.__ixacsProductionDataCache ??= new Map();
...
export function resolveDateKeys(query: ProductionAiDateQuery, today = todayBangkok()) {
  if (query.mode === "range") {
    return isDateKey(query.from) && isDateKey(query.to) ? datesBetween(query.from, query.to) : [];
  }
  if (query.mode === "month") {
...
async function fetchRowsForActiveSession(args: {
  ...
  const lineUuids = discovered.lineUuids.length > 0 ? discovered.lineUuids : connection.lineUuids;
  ...
  const lineMetadata = new Map(
    discovered.groups.flatMap((group) =>
  ...
  const statusMetadata = new Map(
    discovered.statuses.map((status) => [status.uuid, status] as const),
  );
  ...
  const rows = [...returnedLineUuids].map((uuid) => {
    ...
    const detail = detailRows.find((row) => row.uuid === uuid);
    ...
    return {
      ...
      receivedFromDetail: Boolean(detail),
  ...
  return {
    ...
    payloadExtras: {
      ...
      coverage: {
        ...
        complete: missingMonitorLineUuids.length === 0 && missingDetailLineUuids.length === 0,
        monitor: {
...
export type LoadProductionDataResult =
  | { ok: true; payload: ProductionDataPayload; cached: boolean }
  | { ok: false; status: number; payload: ProductionDataPayload };

export async function loadConnectionProductionData(input: {
  connectionId: string;
  dateQuery?: DateQuery;
  customerIds?: unknown;
  fresh?: boolean;
  lock?: boolean;
}): Promise<LoadProductionDataResult> {
  const id = input.connectionId;
  let connection = await getConnection(id);
  if (!connection) return { ok: false, status: 404, payload: { ok: false, error: "Not found", rows: [] } };
  if (!connection.baseUrl) {
    return { ok: false, status: 400, payload: { ok: false, error: "Base URL is required", rows: [] } };
  }

  const today = todayBangkok();
  const requestedDates = resolveDateKeys(input.dateQuery ?? {}, today).filter((date) => date <= today);
  if (requestedDates.length === 0 || requestedDates.length > 3660) {
    return { ok: false, status: 400, payload: { ok: false, error: "Invalid or excessive date range", rows: [] } };
  }
  const historical = requestedDates.length !== 1 || requestedDates[0] !== today;
  const bypassCache = input.fresh === true;
  const bizDates = requestedDates.map((date) => date.split("-").reverse().join("/"));
  const customerIds = resolveRequestedCustomerIds(connection, input.customerIds ?? input.dateQuery?.customerIds);
  if (customerIds.length === 0 && connection.customers.length > 0) {
    return { ok: false, status: 400, payload: { ok: false, error: "CUSTOMER_REQUIRED", rows: [] } };
  }
  ...
  if (!bypassCache && cached && cached.expiresAt > Date.now()) {
    return { ok: true, payload: { ...cached.payload, cached: true }, cached: true };
  }

  const releaseLock = input.lock === false ? () => undefined : await acquireIxacsConnectionLock(id);
  try {
    const cachedAfterWait = dataCache.get(cacheKey);
    if (!bypassCache && cachedAfterWait && cachedAfterWait.expiresAt > Date.now()) {
      return { ok: true, payload: { ...cachedAfterWait.payload, cached: true }, cached: true };
    }
    ...
    for (const customerId of effectiveCustomerIds) {
        ...
        if (!activated.ok) {
          if (allRows.length === 0) {
            return {
              ...
              payload: { ok: false, error: activated.error, customerId, rows: allRows },
            };
          }
          continue;
        }
      ...
      if (!result.ok) {
        if (allRows.length === 0) return { ok: false, status: result.status, payload: result.payload };
        continue;
      }
    ...
    if (allRows.length === 0) {
      return { ok: false, status: 400, payload: { ok: false, error: "NEED_LINES", lineCount: 0, rows: [] } };
    }

    const payload: ProductionDataPayload = {
      ok: true,
      ...(lastExtras ?? {}),
