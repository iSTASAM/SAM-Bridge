...
export async function GET(request: Request) {
  ...
  const response = await readConnectionData(internal, { params: Promise.resolve({ id: connectionId }) });
  ...
  const compact = compactProduction(payload, periodResult.period, companyView, lineId, lineName);
