...
import {
  ...
  type LineLoginCopy,
  type LineLoginErrorCode,
  type LineLoginUiState,
} from "./line-login-copy";
...
function Shell({
  ...
  copy: LineLoginCopy;
  state: LineLoginUiState;
  error: string | null;
  onSubmit: (companyId: string, id: string, password: string) => void;
  onAction: () => void;
}) {
  const form = state === "ready" || state === "submitting" || state === "error";
  const loadingOnly = state === "loading";

  if (loadingOnly) {
    return (
      <div className="line-login-page is-loading-screen">
        <LineLoginStatus copy={copy} state="loading" />
      </div>
    );
  }
  ...
  return (
    <div className="line-login-page">
      <LineLoginHeader copy={copy} />
      <main className="line-login-main">
...
export function LineLoginPage() {
  ...
  const copy = LINE_LOGIN_COPY[locale];
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const allowPreview = Boolean(config.webPreview);

        if (config.configured && config.liffId) {
          try {
            const sdk = await loadLiff();
            ...
            // Fall through to web preview when LIFF init fails outside LINE.
          }
        }
  ...
  async function submit(customerCompanyId: string, loginId: string, password: string) {
    ...
    setError(null);
    try {
      const response = await fetch("/api/line/auth/login", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify(
          webPreview
            ? { customerCompanyId, loginId, password, webPreview: true }
            : {
                customerCompanyId,
                loginId,
                password,
